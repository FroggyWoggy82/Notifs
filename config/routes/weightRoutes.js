 ...
 *         description: Server error
 */
router.get('/calorie-targets/:userId', async (req, res) => {
    try {
        ...
        // Query the database for the user's calorie target
        const result = await db.query(
            'SELECT daily_target FROM calorie_targets WHERE user_id = $1 ORDER BY updated_at DESC LIMIT 1',
            [userId]
        );
        ...
        res.status(500).json({ error: 'Server error getting calorie target' });
    }
});
 ...
 *         description: Server error
 */
router.post('/calorie-targets', async (req, res) => {
    try {
        ...
        // Validate inputs
        if (user_id === undefined || daily_target === undefined) {
        ...
        if (isNaN(userIdNum) || userIdNum < 1 || isNaN(dailyTargetNum) || dailyTargetNum < 500) {
            return res.status(400).json({ error: 'Invalid input values' });
        }

        // Insert the new calorie target
        const result = await db.query(
            'INSERT INTO calorie_targets (user_id, daily_target, updated_at) VALUES ($1, $2, NOW()) RETURNING *',
            [userIdNum, dailyTargetNum]
        );

        res.status(201).json({
            id: result.rows[0].id,
            user_id: userIdNum,
            daily_target: dailyTargetNum,
