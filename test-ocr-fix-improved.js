// Test script for improved OCR decimal point correction
            ...
            if (numValue > 100 && !(numValue === 200 || numValue === 300 || numValue === 400 || numValue === 500 ||
                ...
                if (valueStr.length === 3) {
                    ...
                    if (firstTwoDigits >= 10 && firstTwoDigits <= 99) {
                        ...
                        console.log(`Post-processing: ${numValue} → ${correctedValue} (inserted decimal point after second digit)`);
                        return { value: correctedValue, corrected: true, originalValue: numValue };
                    }
                }
            }
...
// Test cases for calories
console.log('Testing calories cases:');
const caloriesTestCases = [
  { value: 1909, type: 'calories', expected: 190.9 },
  { value: 2728, type: 'calories', expected: 272.8 },
  { value: 1234, type: 'calories', expected: 123.4 },
  { value: 5678, type: 'calories', expected: 567.8 },
  { value: 123, type: 'calories', expected: 12.3 },
  { value: 456, type: 'calories', expected: 45.6 },
  { value: 190.9, type: 'calories', expected: 190.9 },
  { value: 272.8, type: 'calories', expected: 272.8 },
  { value: 500, type: 'calories', expected: 500 },
  { value: 800, type: 'calories', expected: 800 }
];

caloriesTestCases.forEach(test => {
  const result = postProcessNutritionValue(test.value, test.type);
  console.log(`Input: ${test.value} (${typeof test.value}), Result: ${result.value}, Expected: ${test.expected}, Corrected: ${result.corrected}`);
  if (result.value !== test.expected) {
    console.error(`  ERROR: Expected ${test.expected} but got ${result.value}`);
  }
});

// Test cases for macronutrients
console.log('\nTesting macronutrient cases:');
const macroTestCases = [
  { value: 126, type: 'carbs', expected: 12.6 },
  { value: 234, type: 'protein', expected: 23.4 },
  { value: 345, type: 'fat', expected: 34.5 },
  { value: 1234, type: 'protein', expected: 12.34 },
  { value: 55, type: 'protein', expected: 55 },
  { value: 12.6, type: 'carbs', expected: 12.6 },
  { value: 23.4, type: 'protein', expected: 23.4 }
];

macroTestCases.forEach(test => {
  const result = postProcessNutritionValue(test.value, test.type);
  console.log(`Input: ${test.value} (${typeof test.value}), Result: ${result.value}, Expected: ${test.expected}, Corrected: ${result.corrected}`);
  if (result.value !== test.expected) {
    console.error(`  ERROR: Expected ${test.expected} but got ${result.value}`);
