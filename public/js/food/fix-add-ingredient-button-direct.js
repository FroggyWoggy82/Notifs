/**
 * Fix Add Ingredient Button Direct
 * Directly fixes the Add Ingredient button functionality
 */

(function() {
    console.log('[Fix Add Ingredient Button Direct] Initializing');
    ...
    function createNewIngredientRow() {
        ...
        if (!initialized) {
            console.log('[Fix Add Ingredient Button Direct] Creating new ingredient row');
        }
    ...
    function addNewIngredientRow() {
        // Only log once per session
        if (!initialized) {
            console.log('[Fix Add Ingredient Button Direct] Adding new ingredient row');
        }
    ...
    function addEventListenersToIngredientItem(ingredientItem) {
        // Only log once per session
        if (!initialized) {
            console.log('[Fix Add Ingredient Button Direct] Adding event listeners to ingredient item');
        }
    ...
    // Function to initialize all ingredient items
    function initializeIngredientItems() {
        // Only log once per session
        if (!initialized) {
            console.log('[Fix Add Ingredient Button Direct] Initializing ingredient items');
            initialized = true;
        }
        ...
        ingredientItems.forEach(item => {
            addEventListenersToIngredientItem(item);
        });
    }

    // Create a debounced version of initializeIngredientItems
    const debouncedInitializeIngredientItems = debounce(initializeIngredientItems, 500);

    // Initialize when the DOM is loaded
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function() {
            setTimeout(initializeIngredientItems, 500);
        });
    } else {
        setTimeout(initializeIngredientItems, 500);
    }
    ...
    const observer = new MutationObserver(function(mutations) {
        ...
        mutations.forEach(function(mutation) {
            if (mutation.addedNodes.length) {
                // Check if any of the added nodes are ingredient items or contain ingredient items
                for (let i = 0; i < mutation.addedNodes.length; i++) {
                    ...
                    if (node.nodeType === 1) { // Element node
                        ...
                        if (node.querySelector && node.querySelector('.ingredient-item')) {
                            hasRelevantChanges = true;
                            break;
                        }
                    }
        ...
        if (hasRelevantChanges) {
            debouncedInitializeIngredientItems();
        }
    });
    ...
    console.log('[Fix Add Ingredient Button Direct] Initialized');
})();
