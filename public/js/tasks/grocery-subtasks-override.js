/**
 * Grocery Subtasks Override
 * This file completely overrides the grocery subtasks functionality
 */

// Wait for the DOM to be loaded
document.addEventListener('DOMContentLoaded', () => {
    console.log('[OVERRIDE] Grocery subtasks override loaded');
    
    // Override the loadGroceryListSubtasks function
    window.loadGroceryListSubtasks = overrideLoadGroceryListSubtasks;
    
    // Add event listeners for grocery list expand buttons
    setupGroceryListExpandButtons();
    
    // Listen for tasks loaded event
    document.addEventListener('tasksLoaded', setupGroceryListExpandButtons);
    document.addEventListener('tasksRendered', setupGroceryListExpandButtons);
});

/**
 * Set up grocery list expand buttons
 */
function setupGroceryListExpandButtons() {
    console.log('[OVERRIDE] Setting up grocery list expand buttons');
    ...
    console.log(`[OVERRIDE] Found ${groceryTasks.length} grocery list tasks`);
    ...
    groceryTasks.forEach(task => {
        ...
        if (expandButton) {
            ...
            newExpandButton.addEventListener('click', (event) => {
                ...
                console.log(`[OVERRIDE] Expand button clicked for task ${taskId}`);
                overrideLoadGroceryListSubtasks(taskId, task);
            });
        }
    });
}
 ...
 */
function overrideLoadGroceryListSubtasks(taskId, parentElement) {
    console.log(`[OVERRIDE] Loading grocery list subtasks for task ${taskId}`);
    
    // Find or create the subtasks container
    let subtasksContainer = document.querySelector(`.override-subtasks-container[data-parent-id="${taskId}"]`);
    
    if (!subtasksContainer) {
        console.log(`[OVERRIDE] Creating subtasks container for task ${taskId}`);
        subtasksContainer = document.createElement('div');
        subtasksContainer.className = 'override-subtasks-container';
    ...
    if (subtasksContainer.style.display === 'none') {
        console.log(`[OVERRIDE] Showing subtasks for task ${taskId}`);
        ...
        // Generate grocery subtasks if the container is empty
        if (subtasksContainer.children.length === 0) {
            generateGrocerySubtasksUI(taskId, subtasksContainer);
        }
    } else {
        console.log(`[OVERRIDE] Hiding subtasks for task ${taskId}`);
        subtasksContainer.style.display = 'none';
...
function generateGrocerySubtasksUI(taskId, container) {
    console.log(`[OVERRIDE] Generating grocery subtasks UI for task ${taskId}`);
