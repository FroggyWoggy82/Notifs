...
(function() {
        ...
        try {
            ...
            taskItems.forEach((item, index) => {
                ...
                if (titleElement && titleElement.textContent.includes('Yuvi')) {
                    yuviTasks.push(item);
                    console.log('[YUVI-FIX-COMPREHENSIVE] Found Yuvi\'s Bday task at index', index);
                    ...
                    if (!item.classList.contains('completed')) {
                        ...
                        overdueElements.forEach(element => {
                            ...
                            console.log('[YUVI-FIX-COMPREHENSIVE] Fixed overdue date to 5/15/2025');
                        });
                    }
                    ...
                    if (!item.classList.contains('completed')) {
                        ...
                        nextDateElements.forEach(element => {
                            ...
                            if (element.parentNode) {
                                ...
                                console.log('[YUVI-FIX-COMPREHENSIVE] Fixed next date in active task to 5/15/2026');
                            }
                        });
                    }
                    ...
                    if (item.classList.contains('completed')) {
                        ...
                        nextDateElements.forEach(element => {
                            ...
                            if (element.parentNode) {
                                element.parentNode.replaceChild(newElement, element);
                                console.log('[YUVI-FIX-COMPREHENSIVE] Fixed next date in completed task to 5/15/2026');
                            }
                        });
                    }
                    ...
                    if (item.classList.contains('completed')) {
                        ...
                        allElements.forEach(element => {
                            if (element.textContent && (
                                ...
                                if (element.parentNode) {
                                    element.parentNode.replaceChild(newElement, element);
                                    console.log('[YUVI-FIX-COMPREHENSIVE] Fixed duplicate overdue text in completed task');
                                }
                            }
                        });
                    }
            ...
            console.log('[YUVI-FIX-COMPREHENSIVE] Found', yuviTasks.length, 'Yuvi\'s Bday tasks');
            console.log('[YUVI-FIX-COMPREHENSIVE] Fix completed');
        } catch (error) {
            console.error('[YUVI-FIX-COMPREHENSIVE] Error:', error);
        }
    }
    ...
    document.addEventListener('tasksLoaded', function() {
        console.log('[YUVI-FIX-COMPREHENSIVE] Tasks loaded event received');
        setTimeout(fixYuviBdayDates, 500);
    });

    document.addEventListener('taskUpdated', function() {
        console.log('[YUVI-FIX-COMPREHENSIVE] Task updated event received');
        setTimeout(fixYuviBdayDates, 500);
    });

    document.addEventListener('tasksRendered', function() {
        console.log('[YUVI-FIX-COMPREHENSIVE] Tasks rendered event received');
        setTimeout(fixYuviBdayDates, 500);
    });
    ...
    const observer = new MutationObserver(function(mutations) {
        ...
        if (shouldFix) {
            console.log('[YUVI-FIX-COMPREHENSIVE] DOM changes detected, running fix');
